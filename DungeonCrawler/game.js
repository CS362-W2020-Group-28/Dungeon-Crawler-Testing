...
module.exports.Slime =  function Slime(x, y) {
  this.type = "Enemy";
  this.transform = new Transform();
  this.layer = 1;
  ...
  this.Update = function(scene) {
    if (
      !this.active &&
      Math.abs(this.transform.position.x - scene.player.transform.position.x) <
        this.activationRange &&
      Math.abs(this.transform.position.y - scene.player.transform.position.y) <
        this.activationRange &&
      rayCast(scene, this, scene.player)
    ) {
      this.refreshPath(scene);
      this.active = true;
    }
    ...
    if (!this.knockBack) {
      ...
      this.transform.position.y = lerp2(
      ...
      this.transform.position.x = lerp2(
      ...
      if (
        Math.abs(this.transform.position.x - this.kbPos.x) < 1 &&
        Math.abs(this.transform.position.y - this.kbPos.y) < 1
      ) {
        this.knockBack = false;
        this.refreshPath(scene);
      }
  ...
  this.Draw = function(scene) {
    if (this.drawPath) {
      ...
      for (var i = 0; i < this.path.length; i++) {
        ...
        ctx.fillRect(
          Math.floor(
            this.path[i].tile.x * 16 -
              scene.Camera.transform.position.x +
              scene.Camera.offset.x
          ),
          Math.floor(
            this.path[i].tile.y * 16 -
...
module.exports.Skeleton = function Skeleton(x,y) {
